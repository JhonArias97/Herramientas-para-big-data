...
bucket = 'gs://jhon_bucket_llamadas123/'


def main():

    # Leer archivo
    data = get_data(filename = "llamadas123_junio_2022.csv")
    # Extraer resumen
...
def save_data(df_clean):
    ...
    out_name = "Taller_2_Datos_limpios_" + filename
    out_path = os.path.join(bucket, "data", "processed", out_name)
    df_clean.to_csv(out_path)    

    #Guardar Tabla en BigQuery
    df.to_gbq(destination_table='esobigdata.llamada_123', if_exists='append')
